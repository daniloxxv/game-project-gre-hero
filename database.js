...
work (usually artistic) which imitates
brief
still in existence
healthful
...
straightforward; lacking in imagination and spirit
...
showing vulgar in nature or appearance; tawdry
to be involved
...
easy-going trifle
suitably expressed; well chosen; apropos
cut roughly; hired horse
something reverting to an earlier type
...
To solicit customers, votes, or patronage
...
deception
gigantic
a going out; a departure
response
taken, done`

var reg = new RegExp("\\n")
var wordsArr = wordstring.trim().split(reg);
var meaningArr = meaningString.trim().split(reg);
...
function databaseCreator (arr1, arr2) {
    for (var i = 0; i < arr1.length; i++) {
        new Word (arr1[i].toLowerCase(), arr2[i].toLowerCase());
    }
}

databaseCreator(wordsArr, meaningArr);
